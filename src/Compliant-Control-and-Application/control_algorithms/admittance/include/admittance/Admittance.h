...
protected:
  ...
  Vector6d arm_desired_twist_adm_; // 顺应性控制下机械臂的期望速度
...
public:
  // 构造函数和析构函数
  Admittance(ros::NodeHandle &n, double frequency, std::string topic_arm_state,
             std::string topic_arm_command, std::string topic_wrench_state,
             std::vector<double> M, std::vector<double> D,
             std::vector<double> K, std::vector<double> desired_pose,
             std::string base_link, std::string end_link, double arm_max_vel,
             double arm_max_acc);
  ~Admittance() {}
  // 主运行函数
  void run();
