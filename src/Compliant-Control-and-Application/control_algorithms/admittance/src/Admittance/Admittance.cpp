...
Admittance::Admittance(ros::NodeHandle &n, double frequency,
                       ...
                       std::string topic_wrench_state, std::vector<double> M,
                       std::vector<double> D, std::vector<double> K,
                       std::vector<double> desired_pose, std::string base_link,
                       std::string end_link, double arm_max_vel,
                       double arm_max_acc)
    : nh_(n), loop_rate_(frequency), M_(M.data()), D_(D.data()), K_(K.data()),
      desired_pose_(desired_pose.data()), arm_max_vel_(arm_max_vel),
      arm_max_acc_(arm_max_acc), base_link_(base_link), end_link_(end_link) {
  // 初始化订阅器和发布器
  //* 订阅器
  sub_arm_state_ =
      nh_.subscribe(topic_arm_state, 5, &Admittance::state_arm_callback, this,
                    ros::TransportHints().reliable().tcpNoDelay());
  sub_wrench_state_ =
      nh_.subscribe(topic_wrench_state, 5, &Admittance::state_wrench_callback,
  ...
  desired_pose_orientation_.coeffs()
      << desired_pose_.bottomRows(4) / desired_pose_.bottomRows(4).norm();
...
void Admittance::compute_admittance() {
  ...
  Eigen::Quaterniond quat_rot_err(arm_orientation_ *
                                  desired_pose_orientation_.inverse());
  ...
  arm_desired_accelaration =
      M_.inverse() * (-coupling_wrench_arm + wrench_external_);
...
void Admittance::state_arm_callback(
  ...
  // 更新机械臂的位置
  arm_position_ << msg->pose.position.x, msg->pose.position.y,
      msg->pose.position.z;
  // 更新机械臂的方向
  arm_orientation_.coeffs() << msg->pose.orientation.x, msg->pose.orientation.y,
      msg->pose.orientation.z, msg->pose.orientation.w;
  // 更新机械臂的扭力
  arm_twist_ << msg->twist.linear.x, msg->twist.linear.y, msg->twist.linear.z,
...
void Admittance::send_commands_to_robot() {
  ...
  ROS_INFO_STREAM(
      ...
      << "Linear: [" << arm_twist_cmd.linear.x << ", " << arm_twist_cmd.linear.y
      << ", " << arm_twist_cmd.linear.z << "] "
      << "Angular: [" << arm_twist_cmd.angular.x << ", "
      << arm_twist_cmd.angular.y << ", " << arm_twist_cmd.angular.z << "]");
...
bool Admittance::get_rotation_matrix(Matrix6d &rotation_matrix,
  ...
  try {
    ...
    rotation_matrix.setZero();
    ROS_WARN_STREAM_THROTTLE(1, "Waiting for TF from: " << from_frame
                                                        << " to: " << to_frame);
    return false;
  }
  return true;
